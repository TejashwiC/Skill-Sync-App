...
function generateHtml(tests, outputPath) {
    ...
    tests.forEach(test => {
        if (!categoryStats[test.category]) {
            categoryStats[test.category] = { total: 0, passed: 0, failed: 0, duration: 0 };
        }
        ...
        categoryStats[test.category].duration += test.duration;
    });
...
<head>
    ...
    <title>SkillSync Android Appium E2E Execution Report</title>
    ...
    <style>
        ...
        .card.pass { border-top: 4px solid var(--accent-green); }
        .card.fail { border-top: 4px solid var(--accent-red); }
        ...
        .card.pass p { color: var(--accent-green); }
        .card.fail p { color: var(--accent-red); }
...
<body>
    <div class="container">
        <div class="header">
            <h1>📱 SkillSync Appium Test Report</h1>
            <p>Execution finished at ${new Date().toLocaleString()}</p>
        </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        ${t.status === 'Failed' && t.error && t.error !== 'N/A' ? `<div class="error-message">${t.error}</div>` : ''}
                    </td>
                    <td>${t.category}</td>
