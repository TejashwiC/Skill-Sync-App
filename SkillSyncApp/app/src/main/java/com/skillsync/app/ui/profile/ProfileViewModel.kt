...
class ProfileViewModel : ViewModel() {
    ...
    fun uploadProfileImage(base64Image: String) {
        ...
        viewModelScope.launch {
            val result = userRepository.updateProfilePhoto(uid, base64Image)
            if (result.isSuccess) {
                _uploadImageResult.postValue(Result.success(base64Image))
            } else {
                _uploadImageResult.postValue(Result.failure(result.exceptionOrNull() ?: Exception("Unknown error")))
            }
        }
    }
